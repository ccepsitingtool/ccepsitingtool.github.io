...
//TODO Update Maps Style landscape.man_made
var gmap_style=[
  {
...
function drawChoropleth(){
  ...
  queue()
    .defer(d3.csv, "data/dowd_fields.csv")
    .defer(d3.json, "data/sacremento_county_tracts.json")
    .defer(d3.csv, "data/sacremento_county_tracts.csv")
    .defer(d3.csv, "data/dowd_source.csv")
  ...
  function setUpChoropleth(error, fields, dc, choropleth, source) {
    ...
    source_data = source;
    choropleth_data.forEach(function(d, index, data) {
      all_data[d.AFFGEOID] = d;
      all_data[d.AFFGEOID]['AFFGEOID'] = +d.AFFGEOID
      choropleth_data[d.AFFGEOID] = +d.population_total;
      data[index]['AFFGEOID'] = +d.AFFGEOID
    });
...
// });
    ...
    gmap = new google.maps.Map(d3.select("#content").node(), {
      ...
      minZoom: 10,
      maxZoom: 18,
      center: new google.maps.LatLng(38.468594, -121.341461),
    ...
    gmap.setZoom(
      containerHeight < 250 ? 8 :
      containerHeight < 520 ? 9 : 10
    );

    var maxBounds = new google.maps.LatLngBounds(
      new google.maps.LatLng(38.043727,  -121.721052), 
      new google.maps.LatLng(38.799562, -121.027130) 
    );
    ...
    overlay.onAdd = function() {
      ...
      overlay.draw = function() {
        ...
        neighborhoods.selectAll("path")
          ...
          .enter().append("path")
          .attr("d", path)
          .attr("id", function (d) { return "path" + d.properties.AFFGEOID; })
          .attr("class", "nbhd")
          ...
          .style("fill",function(d) {
            var current_id = d.properties.AFFGEOID
            if (currentMetric === null || all_data[current_id][currentMetric] === '0') { return defaultColor; }
            else { return choro_color(all_data[current_id][currentMetric]); }
          })
...
function populateNavPanel(data) {
  ...
  // event listeners for changing ds
  ...
  // school points
  $(".models-menu > li").on("click", "a", function(e){
    e.preventDefault();
...
function changeNeighborhoodData(new_data_column) {
  ...
  choropleth_data.forEach(function(d) {
    choropleth_data[d.AFFGEOID] = +d[new_data_column];
  });
  ...
  g.select("#neighborhoods").selectAll("path")
    ...
    .style("fill", function(d) {
      if(typeof all_data[d.properties.AFFGEOID] ==="undefined" ||
        // all_data[d.properties.AFFGEOID].population_total < min_population ||
        !all_data[d.properties.AFFGEOID][new_data_column] ||
        all_data[d.properties.AFFGEOID][currentMetric] === '0'){
        return defaultColor;
      } else {
        return choro_color(all_data[d.properties.AFFGEOID][new_data_column]);
        // return 'red'
      }
    })
    .style("fill-opacity",0.75);

  if(activeId && new_data_column !== "no_indicator_data") {
...
function drawPoints(type) {
  ...
  var isModel = _.indexOf(['three_day_ID','ten_day_ID','all_points_ID','dropoff_ID'],type) !== -1

  var dataset = type === 'three_day_ID' ? 'three_d_centers' :
                     type ===     'ten_day_ID' ? 'ten_d_centers':
                        type===  'all_points_ID' ? 'all_test_points':
                          type==='dropoff_ID' ? 'dropoff' : null
  ...
  d3.csv('data/' + dataset + '.csv', function (data) {
    ...
    function buildNewSchool($schoolDisplay, school) {
      ...
      $schoolDisplay.find(".school-enrollment").html(getDisplayValue(school.NAME10, "NAME10", "val"));
...
function displayPopBox(d) {
  ...
  var $popbox = $("#pop-info"),
      highlighted = d ? all_data[d.properties.AFFGEOID] : all_data.dc;
...
function highlightNeigborhood(d, isOverlayDraw) {
  ...
  if(!isOverlayDraw) {
    ...
    if (d && all_data[d.properties.AFFGEOID]){
      displayPopBox(d);
      //last neighborhood to display in popBox.
      activeId = d.properties.AFFGEOID;
      setVisMetric(activeData, all_data[activeId][activeData]);
      updateChart(all_data[activeId]);
    }
  } else {
    g.selectAll("#path" + highlightedNeighborhood.properties.AFFGEOID).classed("active", true);
...
function hoverNeighborhood(d) {
  ...
  if (d && all_data[d.properties.AFFGEOID]){
    ...
    if (activeData !== "no_indicator_data") {
      setVisMetric(activeData, all_data[activeId][activeData]);
      updateChart(all_data[activeId]);
    } else {
      setVisMetric(null, null, true);
